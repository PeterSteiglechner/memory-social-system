...
selected_tasks = [10740+l for l in range(20)]
...
for ax, cond in zip(axs.flatten(), conds):
    ...
    for ni, task in enumerate(selected_tasks):
        ...
        taskname = tasks.loc[tasks.task_id==task]["path_to_media"].str.replace("prediction_movie_rating/", "").str.replace(".mp4", "").iloc[0]
        chain = get_chain(task, cond)
        chain["relanswer"].plot(ax=ax, label="_", color=c, alpha=0.2, lw=0, marker="o", markersize=2)
        chain["rollMean"].plot(ax=ax, color=c, label=taskname)
        chain["rollMedian"].plot(ax=ax, color=c, ls="--", label="_")
        # --- right: histogram of language counts ---
        y_grid = np.linspace(min(chain["relanswer"])*1.2, max(chain["relanswer"])*1.2, 100)
...
axs[0,0].set_ylabel("Deviation from y_true [yrs]", y=0)
fig.suptitle("'how-old-is-X' questions")

# %%

# %%
selected_tasks = [10140+l for l in range(20)]
results = []
